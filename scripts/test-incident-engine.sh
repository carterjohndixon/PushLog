...
# Sample GitPush event with critical_paths. Commit touches src/auth/login.ts;
# critical_paths includes "src/auth" so the commit gets "touches critical path" boost.
TEST_EVENT='{
  "source": "pushlog",
  "service": "my-api",
  "environment": "main",
  ...
  "stacktrace": [
    {"file": "src/auth/login.ts", "function": "changed"},
    {"file": "src/auth/utils.ts", "function": "changed"}
  ],
  "links": {"pushlog_user_id": "test-user-123"},
  "change_window": {
    "deploy_time": "2026-02-19T12:00:00Z",
    "commits": [
      {
        "id": "abc123def456",
        "timestamp": "2026-02-19T11:59:00Z",
  ...
  "correlation_hints": {
    ...
    "low_priority_paths": ["docs", "tests"]
  }
}'

if [ -t 0 ]; then
  # No stdin: emit test event and close pipe so engine exits after processing
  echo "$TEST_EVENT" | "$BIN"
else
