...
import { formatLocalDateTime, formatRelativeOrLocal, formatCreatedAt, formatLocalShortDate } from "@/lib/date";
...
export interface UsagePerGenResult {
  generationId: string;
  costUsd: number | null;
...
export interface ProfileUserForModels {
...
export function OpenRouterModels({
  ...
  const fetchUsagePerGenMutation = useMutation({
    mutationFn: async (generationId: string) => {
      const res = await fetch(`/api/openrouter/usage-per-gen/${encodeURIComponent(generationId)}`, {
        credentials: "include",
      });
      const data = await res.json().catch(() => ({}));
    ...
    onSuccess: (data, generationId) => {
      queryClient.setQueryData<UsagePerGenResult>(
        [`/api/openrouter/usage-per-gen/${encodeURIComponent(generationId)}`],
        data
      );
          ...
          <CardContent className="space-y-4">
            ...
            {integrations?.length === 0 && (
              <p className="text-xs text-muted-foreground">
                No integrations yet. Create one from{" "}
                <Link href="/dashboard" className="text-log-green hover:underline">Dashboard</Link> or{" "}
                <Link href="/integrations" className="text-log-green hover:underline">Integrations</Link>.
              </p>
            )}
          </CardContent>
        ...
        <Card className="card-lift mb-8 border-border shadow-forest">
          <CardHeader>
            <div className="flex items-center justify-between">
              ...
              <button
                onClick={() => fetchAllUsageMutation.mutate()}
              ...
              <>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
                  ...
                  <div className="rounded-lg border border-border bg-muted/30 p-4">
                    ...
                    <p className="text-xl font-semibold text-foreground">{(usageData.totalTokens ?? 0).toLocaleString()}</p>
                  </div>
                  <div className="rounded-lg border border-border bg-muted/30 p-4">
                    <p className="text-xs text-muted-foreground uppercase tracking-wide">Estimated cost</p>
                    <p className="text-xl font-semibold text-foreground">{usageData.totalCostFormatted ?? "—"}</p>
                  </div>
                  {creditsLoading ? (
                    <div className="rounded-lg border border-border bg-muted/30 p-4">
                ...
                return (
                  ...
                  >
                    <Star
                      ...
                      onClick={(e) => {
                        e.stopPropagation();
                        toggleFavoriteMutation.mutate(fav.modelId);
                      }}
                    />
                  ...
                  {filteredModels
                    ...
                    .map((m) => (
                      ...
                      >
                          ...
                          <div className="flex items-center gap-2 min-w-0">
                            {userHasKey && (
                              <Star
                                ...
                                onClick={(e) => {
                                  e.stopPropagation();
                                  toggleFavoriteMutation.mutate(m.id);
                                }}
                              />
                            )}
                        ...
                        <TableCell className="text-muted-foreground text-sm">{formatPrice(m.pricing?.prompt)}</TableCell>
                        <TableCell className="text-muted-foreground text-sm">{formatPrice(m.pricing?.completion)}</TableCell>
                        {userHasKey && (
                          <TableCell className="text-muted-foreground text-sm">
          ...
          {fetchUsagePerGenMutation.isPending ? (
            ...
            <div className="space-y-3 text-sm">
              ...
              <p>
                ...
                {usagePerGenResult.tokensPrompt.toLocaleString()}
              </p>
              <p>
                <span className="font-medium text-foreground">Tokens (completion):</span>{" "}
                {usagePerGenResult.tokensCompletion.toLocaleString()}
              </p>
              <p>
                <span className="font-medium text-foreground">Total tokens:</span>{" "}
                {usagePerGenResult.tokensUsed.toLocaleString()}
              </p>
              <a
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                <tr className="border-b border-border/50">
                  ...
                  {Array.from(compareIds).map((id) => (
                    <td key={id} className="p-2 font-mono text-xs text-foreground">
                      {id}
                    </td>
                  ))}
                </tr>
                <tr className="border-b border-border/50">
                  ...
                  {Array.from(compareIds).map((id) => {
                    const m = allModels.find((mod) => mod.id === id);
                    return (
                      <td key={id} className="p-2 text-foreground">
                ...
                <tr>
                  ...
                  {Array.from(compareIds).map((id) => {
                    const m = allModels.find((mod) => mod.id === id);
                    return (
                      <td key={id} className="p-2 text-foreground text-xs">
                        {m?.description?.slice(0, 120) || "—"}
                        {(m?.description?.length ?? 0) > 120 ? "…" : ""}
                      </td>
                    );
                  })}
                </tr>
                ...
                <div className="grid grid-cols-2 gap-3 text-sm">
                  <div className="rounded-lg border border-border bg-muted/30 p-3">
                    ...
                    <p className="font-medium text-foreground">
                      {selectedModel.context_length != null ? selectedModel.context_length.toLocaleString() : "—"}
                    </p>
                  </div>
                  <div className="rounded-lg border border-border bg-muted/30 p-3">
                    ...
                    <div className="rounded-lg border border-border bg-muted/30 p-3 text-sm">
                      ...
                      {selectedModel.top_provider.context_length != null && (
                        <p className="text-foreground">
                          Context: {selectedModel.top_provider.context_length.toLocaleString()}
                        </p>
                      )}
                      {selectedModel.top_provider.max_completion_tokens != null && (
                        <p className="text-foreground">
                ...
                {userHasKey && (
                  <div className="space-y-3">
                    ...
                    {integrations?.length === 0 && (
                      <p className="text-xs text-muted-foreground">
                        No integrations yet. Create one from{" "}
                        <Link href="/dashboard" className="text-log-green hover:underline">
                          Dashboard
                        </Link>{" "}
                        or{" "}
                        <Link href="/integrations" className="text-log-green hover:underline">
                          Integrations
                        </Link>
                        .
                      </p>
